globalThis.monorepoPackagePath = "";globalThis.openNextDebug = false;globalThis.openNextVersion = "3.3.0";
...
var init_cloudflare_node = __esm({
  "node_modules/@opennextjs/aws/dist/overrides/wrappers/cloudflare-node.js"() {
    ...
    handler = async (handler3, converter2) => async (request2, env, ctx) => {
      ...
      ctx.waitUntil(handler3(internalEvent, streamCreator));
      ...
      proxy: (internalEvent) => {
        ...
        return new Promise((resolve, reject) => {
          ...
          const req = request(url, {
            ...
            const nodeReadableStream = _res.headers["content-encoding"] === "br" ? _res.pipe((init_node_zlib(), __toCommonJS(node_zlib_exports)).createBrotliDecompress()) : _res.headers["content-encoding"] === "gzip" ? _res.pipe((init_node_zlib(), __toCommonJS(node_zlib_exports)).createGunzip()) : _res;
...
function fixCacheHeaderForHtmlPages(rawPath, headers) {
...
function createServerResponse(internalEvent, headers, responseStream) {
...
patchAsyncStorage();
async function openNextHandler(internalEvent, responseStreaming) {
  const initialHeaders = internalEvent.headers;
  return runWithOpenNextRequestContext({ isISRRevalidation: initialHeaders["x-isr"] === "1" }, async () => {
    ...
    if ("type" in routingResult) {
      if (responseStreaming) {
        const response = createServerResponse(internalEvent, headers, responseStreaming);
    ...
    const res = createServerResponse(preprocessedEvent, overwrittenResponseHeaders, responseStreaming);
